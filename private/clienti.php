...
if (isset($_POST['edit_id'])) {
    ...
    if ($values['nome'] && $values['cognome']) {
        $stmt = $pdo->prepare('UPDATE cliente SET nome=?, cognome=?, telefono=?, email=?, indirizzo=?, codice_fiscale=?, partita_iva=?, stato=? WHERE id=?');
        $stmt->execute([
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_cliente'])) {
    ...
    if ($values['nome'] && $values['cognome']) {
        $stmt = $pdo->prepare('INSERT INTO cliente (nome, cognome, telefono, email, indirizzo, codice_fiscale, partita_iva, stato) VALUES (?, ?, ?, ?, ?, ?, ?, ?)');
        $stmt->execute([
...
$clienti = $pdo->query('SELECT * FROM cliente ORDER BY cognome, nome')->fetchAll();
?>
<!DOCTYPE html>
<html lang="it">
<head>
...
<div class="form-box" style="text-align:left;">
    <form method="post">
        ...
        <div style="display:flex;gap:1em;flex-wrap:wrap;align-items:flex-start;justify-content:flex-start;">
            ...
            <?php if ($edit_id): ?>
                <a href="clienti.php" style="align-self:flex-end;margin-left:1em;">Annulla</a>
            <?php endif; ?>
        </div>
    </form>
</div>
